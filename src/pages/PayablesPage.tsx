...
const PayablesPage: React.FC = () => {
  ...
  const [selectedSupplier, setSelectedSupplier] = useState<AgingPayable | null>(null);
  ...
  const openPaymentModal = async (supplier: AgingPayable) => {
    if (accounts.length === 0) {
      await fetchAccounts();
    }
    setSelectedSupplier(supplier);
    setPaymentAmount(Number(supplier.total_payable || 0).toFixed(2));
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await axios.post(`${API_BASE_URL}/financial/payables/payment`, {
  ...
  const handleConfirmPayment = async (paymentId: number) => {
    try {
      await axios.post(`${API_BASE_URL}/financial/payables/confirm-payment`, { payment_id: paymentId });
      fetchPendingPayments();
      fetchPayables();
    } catch {}
  };
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold mb-6">Aging Payables</h1>
      {loading ? (
        <div>Loading...</div>
      ) : error ? (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-200 rounded-lg">
            ...
            <tbody>
              {payables.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-4">No outstanding payables.</td>
                </tr>
              ) : (
                payables.map((row) => (
                  <tr key={row.supplier_id}>
                    <td className="px-4 py-2 border-b">{row.company_name}</td>
                    <td className="px-4 py-2 border-b font-semibold">{row.total_payable.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</td>
      ...
      {showModal && selectedSupplier && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md relative">
            ...
            <h2 className="text-lg font-bold mb-4">Make Payment to {selectedSupplier.company_name}</h2>
      ...
      <div className="mt-10">
        <h2 className="text-xl font-bold mb-4">Pending Payments ("in pay")</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-200 rounded-lg">
            <thead>
              <tr>
            ...
            <tbody>
              {pendingPayments.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-4">No pending payments.</td>
                </tr>
              ) : (
                pendingPayments.map((p) => (
                  <tr key={p.id}>
                    <td className="px-4 py-2 border-b">{p.payment_number}</td>
                    <td className="px-4 py-2 border-b">{payables.find(s => s.supplier_id === p.supplier_id)?.company_name || p.supplier_id}</td>
                    <td className="px-4 py-2 border-b">{p.payment_date}</td>
                    <td className="px-4 py-2 border-b">{p.amount.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</td>
                    <td className="px-4 py-2 border-b">{accounts.find(a => a.id === p.account_id)?.account_name || p.account_id}</td>
                    <td className="px-4 py-2 border-b">{p.reference}</td>
